...
async function hoverAndClickAllCopyButtons(page) {
    ...
    const codeBlocks = article.locator(
        'div.theme-code-block, div[class*="codeBlock"], pre',
    );
...
test("Grid.js Docs Flow — ClassName page interactions (FULL, based on prior work)", async ({
    page,
}) => {
    ...
    const configSidebar = page
        .locator('a.menu__link:has-text("Config")')
        .first();
    await expect(configSidebar).toBeVisible();
    await configSidebar.click();
    await waitDocReady(page);

    const classNameSubmenu = page
        .locator('a.menu__link[href="/docs/config/className"]')
        .first();
